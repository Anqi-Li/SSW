...
# -*- coding: utf-8 -*-
"""
Created on Nov 12 12:41:0 2021
...
Sa has no off-diagonal elements
Se has 
...
path = '/home/anqil/Documents/sshfs/oso_extra_storage/StrayLightCorrected/Channel3/'
orbit100 = np.arange(364, 420)
orbit100_idx = int(sys.argv[1])
...
fr_ref = xr.DataArray(fr_ref, dims=('T'), coords=(T_ref,), name='overlap_filter', attrs={'units': '%'})
...
def iteration(forward_fun, fit_fun, y, x_initial, forward_args=(), fit_args=(), max_iter=30):
...
def pathlength_correction(z, h, abs_table, nsteps, background_atm):
    ...
    for ipixel in range(len(h)):
        z_steps, step_size = path_z(z_top=abs_table.z_top.item(), z_tangent=h[ipixel], #in km
                                    sol_zen=90, nsteps=nsteps, extend=True) #in km
        distance_from_z_top = np.arange(len(z_steps)) * step_size # in km
        corr = abs_table.factor.interp(tangent_pressure=background_atm.p.interp(z=h[ipixel]), 
                                       distance=distance_from_z_top)
        layer_indices = z.searchsorted(z_steps)
        u = np.unique(layer_indices[np.where(layer_indices<len(z))])
        pathl_corrected = np.bincount(layer_indices[np.where(layer_indices<len(z))], 
                                           corr[np.where(layer_indices<len(z))]*step_size) #in km
        pathl_uncorrected = np.bincount(layer_indices[np.where(layer_indices<len(z))], 
                                             np.ones(len(layer_indices[np.where(layer_indices<len(z))]))*step_size) #in km
...
def f(i):
    ...
    if alt_chop_cond.sum() < 2:
        print('wrong alt range {}/{} in orbit {}'.format(i-image_lst[0], len(image_lst), orbit_nr))
        pass
    elif l1.isel(mjd=i).notnull().sum() == 0:
        print('l1 is all nan {}/{} in orbit {}'.format(i-image_lst[0], len(image_lst), orbit_nr))
        pass
    elif error.isel(mjd=i).notnull().sum() == 0:
        print('error is all nan {}/{} in orbit {}'.format(i-image_lst[0], len(image_lst), orbit_nr))
        pass
    
    else:
        try:
            print('process image {}/{} in orbit {}'.format(i-image_lst[0], len(image_lst), orbit_nr))
#             print(i)
            
            background_atm = clima.sel(lat=ir.latitude.isel(mjd=i),
            ...
            forward_args = (T_a.values, m_a.values, clima.z.values*1e3, #in m
                            sol_zen, p_a.values)
            ver_clima = forward(o3_clima.values, forward_args)
            ver_a = np.interp(z*1e-3, clima.z, ver_clima)
            
            #make jacobian matrix
            h = altitude.isel(mjd=i).where(l1.isel(mjd=i).notnull(), drop=True).where(alt_chop_cond, drop=True) #in m
            ...
            overlap_filter = fr_ref.interp(T=ir_T, kwargs=dict(fill_value='extrapolate'))*1e-2
...
#            Sa_inv = np.diag(1/(0.75*ver_a)**2)
            ...
            ver = np.interp(z_mr, z_mr[np.where(ver_cond>0)], ver_cond[np.where(ver_cond>0)]) #eliminate neg ver
            ...
            forward_args = (T_a.interp(z=z_mr*1e-3).values, #K
                            ...
                            True) #m
            hour_after_sunrise = cal_sunrise_h(ir.latitude.isel(mjd=i), ir.longitude.isel(mjd=i), ir.mjd[i])
            if np.isnan(hour_after_sunrise):
                scale_lifetime = 1
            else:   
                scale_lifetime = (1 - np.exp(-hour_after_sunrise / lifetime.interp(z=z_mr*1e-3)))**4
...
#            Se_inv = np.linalg.inv(Se) #full Se matrix
            Se_inv = np.diag(scale_lifetime**2 / ver_meas_error2[cond_ver_mr]) #only diaganol
            D = Sa_inv #for Levenberg Marquardt
            fit_args = (Se_inv, Sa_inv, o3_a, D)
            
            LM_result = iteration(forward, fit, ver, o3_initial, forward_args=forward_args, fit_args=fit_args)
            ...
            o3_meas_error2 = xr.DataArray(o3_meas_error2, coords={'z': z_mr}, dims='z').reindex(z=z)
            o3_smooth_error2 = xr.DataArray(o3_smooth_error2, coords={'z': z_mr}, dims='z').reindex(z=z)
            limb_residual = xr.DataArray(limb_residual, coords={'pixel': h.pixel}, dims='pixel').reindex(pixel=l1.pixel)
                                    
            return (day_mjd_lst[i].values,
                    # ver retrieval
                    ver_hat, ver_mr, ver_mr_rel, np.sqrt(ver_meas_error2), np.sqrt(ver_smooth_error2), 
                    limb_residual, ver_cost_x, ver_cost_y,
                    # o3 retrieval
                    o3_hat, o3_mr, o3_mr_rel, np.sqrt(o3_meas_error2), np.sqrt(o3_smooth_error2), 
                    ver_residual, o3_cost_x, o3_cost_y, o3_n_evaluate, o3_gamma, o3_status, 
                    # apriori
                    o3_clima.values, ver_clima, 
                    # the others
                    hour_after_sunrise,
                    ver_A_rel, o3_A_rel, # not saving to nc file
                    ver_A, o3_A #not saving to nc file
                    )
...
def run_and_save(image_lst, orbit_nr):
    ...
    if len(result)>0:
        ...
        ds = ds.update({
            ...
            # ver retrieval
            'ver': (['mjd', 'z'], np.stack([result[i][1] for i in range(len(result))]), {'units': 'photons cm-3 s-1'}),
            'ver_mr':(['mjd', 'z'], np.stack([result[i][2] for i in range(len(result))])), 
            'ver_mr_rel':(['mjd', 'z'], np.stack([result[i][3] for i in range(len(result))])),
            'ver_retrieval_error':(['mjd', 'z'], np.stack([result[i][4] for i in range(len(result))]), {'units': 'photons cm-3 s-1'}),
            'ver_smoothing_error':(['mjd', 'z'], np.stack([result[i][5] for i in range(len(result))]), {'units': 'photons cm-3 s-1'}),
            'limb_residual':(['mjd', 'pixel'], np.stack([result[i][6] for i in range(len(result))]), {'units': 'photons cm-3 s-1'}),            
            'ver_cost_x':(['mjd'], np.stack([result[i][7] for i in range(len(result))])), 
            'ver_cost_y':(['mjd'], np.stack([result[i][8] for i in range(len(result))])),  
            # o3 retrieval
            'o3':(['mjd', 'z'], np.stack([result[i][9] for i in range(len(result))]), {'units': 'cm-3'}),              
            'o3_mr':(['mjd', 'z'], np.stack([result[i][10] for i in range(len(result))])), 
            'o3_mr_rel':(['mjd', 'z'], np.stack([result[i][11] for i in range(len(result))])),
            'o3_retrieval_error':(['mjd', 'z'], np.stack([result[i][12] for i in range(len(result))]), {'units': 'cm-3'}),
            'o3_smoothing_error':(['mjd', 'z'], np.stack([result[i][13] for i in range(len(result))]), {'units': 'cm-3'}),
            'ver_residual':(['mjd', 'z'], np.stack([result[i][14] for i in range(len(result))]), {'units': 'photons cm-3 s-1'}),            
            'o3_cost_x':(['mjd'], np.stack([result[i][15] for i in range(len(result))])), 
            'o3_cost_y':(['mjd'], np.stack([result[i][16] for i in range(len(result))])),
            'o3_evaluate':(['mjd'], np.stack([result[i][17] for i in range(len(result))]), {'note': 'number of iterations'}),  
            'o3_gamma':(['mjd'], np.stack([result[i][18] for i in range(len(result))])),  
            'o3_status':(['mjd'], np.stack([result[i][19] for i in range(len(result))]), {'note': '1:converged by xchange, 2:converged by d2, 3:diverged'}), 
            # apriori
            'o3_a': (['mjd', 'clima_z'], np.stack([result[i][20] for i in range(len(result))]), {'units': 'cm-3', 'name':'CMAM O3'}),
            'ver_a':(['mjd', 'clima_z'], np.stack([result[i][21] for i in range(len(result))]), {'units': 'photons cm-3 s-1'}),
            # the others
            'hour_after_sunrise': (['mjd'], np.stack([result[i][22] for i in range(len(result))])),
        ...
        ds.to_netcdf('~/Documents/osiris_database/iris_ver_o3/ver_o3/v2p0/iri_ver_o3_orbit{}_v2p0.nc'.format(orbit_nr))

#%%%%%%%%%%%%%%%%%%%% main part #####################
if __name__ == '__main__':
    for orbit_nr in np.unique(ir.orbit)[::2]: #process every *th orbit
        if os.path.exists('/home/anqil/Documents/osiris_database/iris_ver_o3/ver_o3/v2p0/iri_ver_o3_orbit{}_v2p0.nc'.format(orbit_nr)):
            print('Orbit {} already have been processed and saved'.format(orbit_nr))
        else:  
            image_lst = ir.mjd.searchsorted(ir.where(ir.orbit == orbit_nr, drop=True).mjd)
